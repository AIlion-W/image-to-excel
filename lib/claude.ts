...
5. 箱数 - 【必填字段】标准格式下方一行开头是"X件 × ..."，这个"件"字一定存在，箱数一定能找到。【三步提取法】第一步：在第二行中间部分仔细找到"件"字的位置（手写"件"字由单人旁"亻"和右侧笔画组成，4个笔画）。第二步：找到"件"字后，将其位置标记为分割点，排除"件"字本身。第三步：提取"件"字左边的所有数字作为箱数。【强制纠错】如果看到"×"符号后面出现了数字，说明已经超过"件"字了，要回头重新定位"件"字。不能轻易标[待确认]，只有在极端情况下（图片模糊不清）才能标。
   【"件"字误读纠错】"件"字绝对不能被识别成数字！常见误读：识别成"-14""4""24""74"都是错的，这些都是把"件"字的笔画拆成了数字。箱数单元格里只能出现纯数字，不能包含"件"字本身。
...
- 箱数字段注意：手写"件"字的单人旁"亻"容易被误认成"4"，但箱数本身可以是任意数字（个位或两位数都有可能），不要强制截断。只在末尾的"4"明显是"件"字笔画时才去掉。"件"字不能被识别成-14、4、24、74等数字。箱数单元格只填纯数字，不含"件"字。"X款"是款数不是箱数，应放做货要求
